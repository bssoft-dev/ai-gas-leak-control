...
class ServiceUIServer:
    ...
    def _setup_routes(self) -> None:
        ...
        async def index_html():
            """서비스 워커 precache 등에서 `/index.html`을 요청할 때 `/`와 동일하게 제공."""
            return await index()
        ...
        def _first_interface_dist_dir() -> Optional[Path]:
            """첫 번째 인터페이스 binding 기준 빌드 출력 디렉터리(dist 등)."""
            binding_path = _first_interface_binding_path(self)
            if not binding_path or not binding_path.exists():
                return None
            for build_dir in ["dist", "build", "out", ".next"]:
                p = binding_path / build_dir
                if p.exists() and p.is_dir():
                    return p
            return None

        @self.app.get("/manifest.json", response_class=FileResponse)
        async def serve_manifest_json():
            """PWA manifest (루트 경로 — PWABuilder / 브라우저 연동)."""
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
        ...
        @self.app.get("/manifest.webmanifest", response_class=FileResponse)
        async def serve_manifest_webmanifest():
            """PWA manifest (대체 확장자)."""
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            file_path = dist_dir / "manifest.webmanifest"
            if not file_path.is_file():
                raise HTTPException(status_code=404, detail="Not found")
            return FileResponse(str(file_path), media_type="application/manifest+json")

        @self.app.get("/sw.js", response_class=FileResponse)
        ...
        @self.app.get("/offline.html", response_class=FileResponse)
        async def serve_offline_html():
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            file_path = dist_dir / "offline.html"
            if not file_path.is_file():
                raise HTTPException(status_code=404, detail="Not found")
            return FileResponse(str(file_path), media_type="text/html; charset=utf-8")

        @self.app.get("/icons/{path:path}", response_class=FileResponse)
        async def serve_pwa_icons(path: str):
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            base = (dist_dir / "icons").resolve()
        ...
        @self.app.get("/screenshots/{path:path}", response_class=FileResponse)
        async def serve_pwa_screenshots(path: str):
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            base = (dist_dir / "screenshots").resolve()
            file_path = (base / path).resolve()
            if not str(file_path).startswith(str(base)):
                raise HTTPException(status_code=403, detail="Forbidden")
            if not file_path.exists() or not file_path.is_file():
        ...
        @self.app.get("/js/{path:path}", response_class=FileResponse)
        async def serve_dist_js(path: str):
            """프론트 dist/js (예: JSPrintManager.js) 정적 서빙."""
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            base = (dist_dir / "js").resolve()
            file_path = (base / path).resolve()
            if not str(file_path).startswith(str(base)):
                raise HTTPException(status_code=403, detail="Forbidden")
            if not file_path.exists() or not file_path.is_file():
                raise HTTPException(status_code=404, detail="Not found")
            media_type = _media_type_for_path(file_path) or "application/javascript"
            return FileResponse(str(file_path), media_type=media_type)

        @self.app.get("/download/android")
        async def serve_android_apk_download():
            """안드로이드 APK 다운로드: 프론트 dist의 download/android.apk → GET /download/android"""
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            dl_dir = (dist_dir / "download").resolve()
            file_path = (dl_dir / "android.apk").resolve()
            if not str(file_path).startswith(str(dl_dir)):
                raise HTTPException(status_code=403, detail="Forbidden")
            if not file_path.is_file():
                raise HTTPException(status_code=404, detail="Not found")
            return FileResponse(
        ...
        @self.app.get("/{filename}", response_class=FileResponse)
        async def serve_root_static_file(filename: str):
            """dist 디렉터리 루트에 빌드된 정적 파일(예: oauth-callback.html, apple-touch-icon.svg) 서빙."""
            if filename in ("api", "static", "assets", "src", "icons", "screenshots", "js", "download"):
                raise HTTPException(status_code=404, detail="Not found")
            dist_dir = _first_interface_dist_dir()
            if not dist_dir:
                raise HTTPException(status_code=404, detail="Not found")
            dist_dir_resolved = dist_dir.resolve()
            file_path = (dist_dir_resolved / filename).resolve()
            prefix = str(dist_dir_resolved)
            if not prefix.endswith(os.sep):
                prefix += os.sep
            if not str(file_path).startswith(prefix):
                raise HTTPException(status_code=403, detail="Forbidden")
            if file_path.is_file():
                media_type = _media_type_for_path(file_path) or "application/octet-stream"
                if filename.endswith(".html"):
                    media_type = "text/html; charset=utf-8"
                return FileResponse(str(file_path), media_type=media_type)
            raise HTTPException(status_code=404, detail="Not found")
        if self.service_def and self.service_def.metadata.id == "com.SagoHub.dangbae":
            @self.app.get("/api/dangbae-order-item-image", response_class=FileResponse)
            async def serve_dangbae_order_item_image(p: str = Query(..., min_length=3, max_length=2048)):
                """주문 물품 이미지 파일 서빙 (저장 경로: services/dangbae/data/order_item_images/…)."""
                rel = p.replace("\\", "/").strip()
                if ".." in rel or rel.startswith("/"):
                    raise HTTPException(status_code=403, detail="Forbidden")
                project_root = self.services_dir.parent.resolve()
                base = (project_root / "services" / "dangbae" / "data" / "order_item_images").resolve()
        ...
        if self.service_id == "com.SagoHub.ai-gas-leak-control":
            ...
            async def get_gas_leak_timeseries():
                ...
                default_ts = {"pressure": [], "flow": [], "flow_diff": [], "sensors": {}}
            ...
            async def get_gas_leak_drawing(drawing_id: str):
                ...
                try:
                    ...
                    valves = []
                    if points_path.exists():
                        raw_pts = json.loads(points_path.read_text(encoding="utf-8"))
                        if isinstance(raw_pts, dict):
                            sensors = raw_pts.get("sensors") if isinstance(raw_pts.get("sensors"), list) else []
                            valves = raw_pts.get("valves") if isinstance(raw_pts.get("valves"), list) else []
            ...
            _gas_edge_registry_path = _gas_leak_dir / "edge_registry.json"

            @self.app.get("/api/gas-leak/edges")
            async def get_gas_leak_edges():
                """등록된 엣지 컨트롤러 목록 (edge_registry.json)"""
                if not _gas_edge_registry_path.exists():
                    return []
                try:
                    data = json.loads(_gas_edge_registry_path.read_text(encoding="utf-8"))
                    ...
                    return [
                        {
                            "edge_id": edge_id,
                            "ip": info.get("ip"),
                            "port": info.get("port", 26030),
                            "registered_at": info.get("registered_at"),
                        }
                        for edge_id, info in data.items()
                        if isinstance(info, dict)
                    ]
                except Exception:
                    return []

            @self.app.get("/api/gas-leak/available-sensors")
            async def get_gas_leak_available_sensors(edge_id: str = None):
                """현재 데이터가 수신되고 있는 센서 ID 목록 (edge_id 필터 가능)"""
                if not _gas_timeseries_path.exists():
                    return []
                try:
                    data = json.loads(_gas_timeseries_path.read_text(encoding="utf-8"))
                    ...
                    # 엣지 ID로 필터링 (센서 ID가 edge_id/sensor_id 형식이라고 가정하거나, 
                    # 또는 그냥 전체 목록 반환 후 프론트에서 처리)
                    sensor_ids = list(sensors_dict.keys())
                    if edge_id:
                        # 엣지 ID와 매칭되는 센서만 반환 (구현에 따라 다름)
                        # 여기서는 단순 포함 여부나 별도 로직이 필요할 수 있음
                        # 일단은 전체를 보내고 프론트에서 처리하도록 하거나, 
                        # 센서 ID 규칙이 있다면 그에 맞춰 필터링
                        pass
                    return sensor_ids
                except Exception:
                    return []

            def _gas_read_json(name: str, default):
                fp = _gas_leak_dir / name
                if not fp.exists():
                    return default
                try:
            ...
            async def post_gas_leak_policy(request: Request):
                ...
                try:
                    ...
                    r.raise_for_status()
                    return {"status": "success", "policy": body}
                except Exception as e:
                    raise HTTPException(status_code=500, detail=str(e))

            @self.app.get("/api/gas-leak/control-history")
            async def get_gas_leak_control_history(limit: int = 200):
                data = _gas_read_json("control_history.json", [])
                if not isinstance(data, list):
                    return []
                return list(reversed(data[-limit:]))

            @self.app.get("/api/gas-leak/alarm-history")
            async def get_gas_leak_alarm_history(limit: int = 200):
                data = _gas_read_json("alarm_history.json", [])
                if not isinstance(data, list):
                    return []
                return list(reversed(data[-limit:]))
            ...
            async def get_gas_leak_daily_usage():
                doc = _gas_read_json("daily_usage.json", {"date": None, "cumulative_liters": 0})
                state = _gas_read_json("state.json", {})
                return {
                    "date": doc.get("date"),
                    "cumulative_liters": doc.get("cumulative_liters", 0),
                    "today_display": state.get("daily_gas_liters", doc.get("cumulative_liters", 0)),
                }

            @self.app.get("/api/gas-leak/flow-diff")
            async def get_gas_leak_flow_diff():
                ts = _gas_read_json("timeseries.json", {})
                return {
                    "series": ts.get("flow_diff") or [],
                    "latest": (_gas_read_json("state.json", {}) or {}).get("flow_diff_latest", 0),
                }

            @self.app.post("/api/gas-leak/edges/register")
            async def post_gas_leak_edge_register(request: Request):
                import requests
                try:
                    body = await request.json()
                except Exception:
                ...
                try:
                    r = requests.post(
                        f"{self.event_bus_url.rstrip('/')}/publish",
                        json={"type": "GAS_LEAK_EDGE_REGISTER", "payload": body},
                        timeout=10,
                    )
                    r.raise_for_status()
                    return {"status": "success"}
            ...
            @self.app.get("/api/gas-leak/edges/{edge_id}/status")
            async def get_gas_leak_edge_status(edge_id: str):
                status = _gas_read_json("edge_status.json", {})
                state = _gas_read_json("state.json", {})
                info = status.get(edge_id) if isinstance(status, dict) else None
                return {
                    "edge_id": edge_id,
                    "online": bool(info and info.get("online")),
                    "load_percent": (info or {}).get("load_percent", state.get("edge_load_percent", 0)),
                    "last_seen": (info or {}).get("last_seen", state.get("edge_last_heartbeat")),
                }

            @self.app.get("/api/gas-leak/statistics/usage-by-area")
            async def get_gas_leak_usage_by_area():
                sensors = _gas_read_json("sensors.json", [])
                zones: Dict[str, float] = {}
                for s in sensors if isinstance(sensors, list) else []:
                    z = s.get("zone_id") or "default"
                    zones[z] = zones.get(z, 0) + float(s.get("value") or 0)
                return [{"zone_id": k, "usage": round(v, 2)} for k, v in zones.items()]

            @self.app.get("/api/gas-leak/statistics/year-over-year")
            async def get_gas_leak_yoy_savings():
                daily = _gas_read_json("daily_usage.json", {"cumulative_liters": 0})
                current = float(daily.get("cumulative_liters", 0))
                baseline = current * 1.12
                savings_pct = round((1 - current / baseline) * 100, 1) if baseline else 0
                return {"current_liters": current, "baseline_liters": round(baseline, 2), "savings_percent": savings_pct}

            @self.app.get("/api/gas-leak/sop")
            async def get_gas_leak_sop():
                return _gas_read_json("sop.json", {"title": "비상 대응 SOP", "steps": []})

            @self.app.post("/api/gas-leak/sop")
            async def post_gas_leak_sop(request: Request):
                try:
                    body = await request.json()
                except Exception:
                    body = {}
                fp = _gas_leak_dir / "sop.json"
                fp.parent.mkdir(parents=True, exist_ok=True)
                fp.write_text(json.dumps(body, ensure_ascii=False, indent=2), encoding="utf-8")
                return {"status": "success"}

            @self.app.get("/api/gas-leak/widget-layout")
            async def get_gas_leak_widget_layout():
                return _gas_read_json("widget_layout.json", {"widgets": []})

            @self.app.post("/api/gas-leak/widget-layout")
            async def post_gas_leak_widget_layout(request: Request):
                try:
                    body = await request.json()
                except Exception:
                    body = {"widgets": []}
                fp = _gas_leak_dir / "widget_layout.json"
                fp.write_text(json.dumps(body, ensure_ascii=False, indent=2), encoding="utf-8")
                return {"status": "success"}

            @self.app.get("/api/gas-leak/plc-devices")
            async def get_gas_leak_plc_devices():
                return _gas_read_json("plc_devices.json", [])

            @self.app.post("/api/gas-leak/plc-devices")
            async def post_gas_leak_plc_devices(request: Request):
                try:
                    body = await request.json()
                except Exception:
                    body = []
                fp = _gas_leak_dir / "plc_devices.json"
                fp.write_text(json.dumps(body, ensure_ascii=False, indent=2), encoding="utf-8")
                return {"status": "success"}

            @self.app.get("/api/gas-leak/maintenance")
            async def get_gas_leak_maintenance():
                return _gas_read_json("maintenance.json", [])

            @self.app.post("/api/gas-leak/maintenance")
            async def post_gas_leak_maintenance(request: Request):
                import requests
                try:
                    body = await request.json()
                except Exception:
                    body = {}
                hist = _gas_read_json("maintenance.json", [])
                if not isinstance(hist, list):
                    hist = []
                body["at"] = body.get("at") or datetime.utcnow().isoformat() + "Z"
                hist.append(body)
                fp = _gas_leak_dir / "maintenance.json"
                fp.write_text(json.dumps(hist[-500:], ensure_ascii=False, indent=2), encoding="utf-8")
                return {"status": "success"}

            @self.app.get("/api/gas-leak/ai-models")
            async def get_gas_leak_ai_models():
                return _gas_read_json("ai_models.json", {"active_version": "v1.0.0-mock", "versions": []})

            @self.app.post("/api/gas-leak/ai-feedback")
            async def post_gas_leak_ai_feedback(request: Request):
                import requests
                try:
                    body = await request.json()
                except Exception:
                ...
                try:
                    r = requests.post(
                        f"{self.event_bus_url.rstrip('/')}/publish",
                        json={"type": "GAS_LEAK_AI_FEEDBACK", "payload": body},
                        timeout=10,
                    )
                    r.raise_for_status()
                    return {"status": "success"}
            ...
            @self.app.get("/api/gas-leak/cctv/demo")
            async def get_gas_leak_cctv_demo():
                return {"url": None, "message": "CCTV 연동 데모 — 실제 RTSP URL은 edge_registry와 연동 예정"}

            @self.app.get("/api/gas-leak/mobile/manifest")
            async def get_gas_leak_mobile_manifest():
                return {
                    "name": "SagoHub 가스누출 모바일",
                    "short_name": "GasLeak",
                    "start_url": "/",
                    "display": "standalone",
                    "features": [
                        "biometric_login_stub",
                        "fcm_push_stub",
                        "sensor_status",
                        "emergency_alert",
                        "valve_remote_control",
                        "control_result",
        ...
        if self.service_id == "com.SagoHub.qr-drawing-manager":
            ...
            @self.app.post("/api/drawing-qr/projects/{project_id}/pages/bulk-delete")
            async def post_drawing_qr_project_pages_bulk_delete(project_id: str, request: Request):
                """프로젝트에 속한 도면(페이지) 여러 개를 한 번에 삭제."""
                if not _is_uuid_like(project_id or ""):
                    raise HTTPException(status_code=400, detail="Invalid project_id")
                ...
                for pid in unique_ids:
                    ...
                    uid = page.get("upload_id")
                    if uid not in upload_set:
                        raise HTTPException(status_code=400, detail="page not in this project")

                    to_remove.append(page)

                remove_ids = {p.get("id") for p in to_remove if p.get("id")}
                cat["pages"] = [p for p in pages if p.get("id") not in remove_ids]
                cat["relations"] = [
                    r
                    for r in (cat.get("relations") or [])
                    if r.get("anchor_page_id") not in remove_ids and r.get("page_id") not in remove_ids
                ]
                remaining_group_ids = set(p.get("group_id") for p in cat["pages"] if p.get("group_id"))
                cat["groups"] = [g for g in (cat.get("groups") or []) if g.get("id") in remaining_group_ids]
                ...
                return {"status": "success", "deleted": len(to_remove)}
