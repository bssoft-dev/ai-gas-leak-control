...
export default function MonitorPage() {
  ...
  const { seriesBySensorId, error: pressureSeriesError } = useMonitorPressureSeries(2000)

  /**
   * 도면 센서의 id 또는 label로 시리즈를 조회합니다.
   * 우선순위: sensor.id → sensor.label
   */
  const getSeriesForSensor = useCallback(
    (sensorId: string, sensorLabel?: string) =>
      seriesBySensorId.get(sensorId) ??
      (sensorLabel ? seriesBySensorId.get(sensorLabel) : undefined),
    [seriesBySensorId],
  )
  const {
    drawings,
  ...
  // detailCard의 도면 센서 정보를 가져와 시리즈 조회
  const detailDrawingSensor = detailCard
    ? (activeDrawing?.sensors ?? []).find((s) => s.id === detailCard.id)
    : undefined
  const detailSeries = detailCard
    ? getSeriesForSensor(detailCard.id, detailDrawingSensor?.label)
    : undefined
          ...
          <div className="mt-[8px] flex min-h-0 flex-1 flex-col">
            <div className="flex max-h-[520px] min-h-[320px] min-w-0 flex-1 flex-col overflow-hidden rounded-[8px] bg-white shadow-[0px_1px_2px_0px_rgba(0,0,0,0.12),0px_1px_3px_1px_rgba(0,0,0,0.05)] md:max-h-[640px] xl:max-h-none xl:min-h-[786px]">
              <div className="notion-scrollbar flex min-h-0 flex-1 flex-col gap-3 overflow-y-auto pt-[12px] pb-[8px] px-[6px] sm:px-[8px]">
                ...
                {cards.map((card) => {
                  // 도면 센서 정보에서 label 추출 (id → label fallback 조회)
                  const drawingSensor = (activeDrawing?.sensors ?? []).find((s) => s.id === card.id)
                  const series = getSeriesForSensor(card.id, drawingSensor?.label)
