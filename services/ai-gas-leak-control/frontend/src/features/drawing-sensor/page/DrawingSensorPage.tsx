...
import { useEdgeList } from '../model/useEdgeList'
import { useAvailableSensors } from '../model/useAvailableSensors'
...
export default function DrawingSensorPage() {
  ...
  const { edges } = useEdgeList(5000)
  const [selectedEdgeId, setSelectedEdgeId] = useState<string | null>(null)
  const { sensorIds: availableSensorIds } = useAvailableSensors(selectedEdgeId)

  // 엣지가 로드되면 첫 번째를 자동 선택
  useEffect(() => {
    if (edges.length > 0 && !selectedEdgeId) {
        ...
        payload: {
          ...
          sensors: nextSensors.map((sensor) => {
            ...
            return {
              ...
              edge_id: sensor.edgeId ?? selectedEdgeId ?? null,
            }
          }),
        },
      })
    ...
    <>
      ...
      <PageContentGrid>
        ...
        <SensorManagementSidebar
          ...
          onEdgeChange={setSelectedEdgeId}
          onAdd={onAdd}
          onCancel={onCancel}
        ...
        <div className="col-span-12 mt-[12px] flex shrink-0 items-center justify-center gap-[36px] text-[16px] text-[#8ea1bb] md:gap-[51px] xl:col-span-9">
